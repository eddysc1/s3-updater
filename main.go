...
func downloadBucket(profile, bucket string) {
	...
	fmt.Println("Download completed successfully. Modify contents in /tmp/s3-updater and choose to reupload in the next step.")
}
...
func uploadBucket(profile, bucket string) {
	cmd := exec.Command("aws", "s3", "sync", "/tmp/s3-updater", fmt.Sprintf("s3://%s", bucket), "--profile", profile)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	err := cmd.Run()
	if err != nil {
